...
def query_source_db(keyword, database):
    ...
    c = connection.cursor()
    k = ("%%%s%%" % keyword) * 5
    k = {"keyword": "%{}%".format(keyword)}
    c.execute(
        """SELECT ZABCDEMAILADDRESS.ZADDRESS, ZABCDRECORD.ZTITLE,
              ...
              WHERE ZABCDEMAILADDRESS.ZADDRESS LIKE :keyword
              OR ZABCDRECORD.ZFIRSTNAME LIKE :keyword
              OR ZABCDRECORD.ZMIDDLENAME LIKE :keyword
              OR ZABCDRECORD.ZLASTNAME LIKE :keyword
              OR ZABCDRECORD.ZORGANIZATION LIKE :keyword""",
        k,
    )
